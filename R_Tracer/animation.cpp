...
----------------------------------------------------------------------*/
Animation* FindAnimation(std::string name,struct AnimationList *al)
{
    struct AnimationList* pObj = al;

    while(pObj != 0)
    {
        if(std::strcmp((pObj->animation).name, name.c_str()) == 0)
...
/*----------------------------------------------------------------------
  GetCamera()
  Purpose: to get the position, the viex    w direction and the up vector
